...
window.initSOPApp = function () {
    ...
    const ExportModule = {
        ...
        print() {
            ...
            const originalWrapperStyle = UtilsModule.$('preview-wrapper').style.cssText;

            // 2. UNLOCK LAYOUT (Force browser to see full document flow)
            document.body.style.cssText = "height: auto !important; overflow: visible !important; position: static !important; display: block !important;";
        ...
        async generatePDF(filename) {
            if (confirm('For best quality and editable text, please choose "Save as PDF" in the print dialog.\n\nClick OK to open Print Dialog, or Cancel to use the image-based generator.')) {
                this.print();
                return;
            }
        ...
        async saveDOCX(filename) {
            ...
            if (typeof htmlDocx === 'undefined') {
                alert('DOCX library (html-docx-js) not found. Add the script tag to index.html.');
                return;
            }
                    ...
                    <div style="max-width: 600px; margin: 40px auto; padding: 30px; background: #fff3cd; border: 2px solid #ffc107; border-radius: 8px; font-family: system-ui, sans-serif;">
                        ...
                        <details style="margin-top: 20px; font-size: 14px;">
                            ...
                            <ul style="margin-top: 10px; line-height: 2; color: #555;">
                                ...
                                <li><strong>Path Auto-Detect:</strong> ${pathResolved ? '✅ Success' : '❌ Failed (using fallback)'}</li>
                                <li><strong>Error Type:</strong> ${e.name}</li>
                                <li><strong>Stack:</strong> <pre style="background: #f8f9fa; padding: 10px; overflow: auto; font-size: 11px;">${e.stack || 'N/A'}</pre></li>
                            </ul>
                        </details>
